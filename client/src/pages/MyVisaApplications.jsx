      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {applications.map(app => (
          <div key={app._id} className="card shadow-lg compact bg-base-100">
            <figure>
              <img src={app.country_image} alt={app.country_name} className="w-full h-48 object-cover"/>
            </figure>
            <div className="card-body">
              ...
              <p>Applicant's Name: {app.first_name} {app.last_name}</p>
              <p>Applicant’s Email: {app.email}</p>
              <div className="badge badge-info mb-2">Status: {app.status}</div>
              <div className="card-actions justify-end">
                <button className="btn btn-error" onClick={() => handleCancel(app._id)}>Cancel</button>
              </div>
            </div>
          </div>
        ))}
